...
\texttt{mul\{S\}}  & \Rc/\Mc & --- & ---
  & Unsigned mul: RDX:RAX = RAX $\times$ src \\
\texttt{div\{S\}}  & \Rc/\Mc & --- & ---
  & Unsigned divide: RAX=quot, RDX=rem \\
\texttt{idiv\{S\}} & \Rc/\Mc & --- & ---
  & Signed divide: RAX=quot, RDX=rem \\
...
\texttt{pop\{S\}}  & \Rc         & --- & --- & Pop from stack  (RSP $+=$ 8) \\
\midrule
...
\texttt{pop}       & \Rc         & --- & --- & Pop from stack  (RSP $+=$ 8) \\
\midrule
\texttt{jmp}       & \Lc/\Mc/\Rc & --- & --- & Unconditional jump \\
\texttt{jcc}       & \Lc         & --- & --- & Conditional jump (see below) \\
\texttt{call}      & \Lc/\Mc/\Rc & --- & --- & Call subroutine \\
...
{
\footnotesize
\begin{tabular}{@{}l l l p{3.2cm} p{3.2cm} p{3.2cm} p{1.5cm}@{}}
\toprule
...
XMM6--7 & ---     & Yes / No
  & SSE 128-bit vector
  & \textbf{Callee-saved} (XMM6--7)
  & Caller-saved (XMM6--15)
  & Win preserves 6--7 \\
\midrule
RFLAGS & flags    & ---
...
\texttt{q}  & Any 64-bit GPR                & 8
  & \texttt{x}  & Any SSE reg (XMM)            & 16 \\
\texttt{l}  & Any register (legacy)         & any
